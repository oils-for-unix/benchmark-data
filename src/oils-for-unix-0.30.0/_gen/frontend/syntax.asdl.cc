...
hnode_t* ShFunction::PrettyTree(bool do_abbrev, Dict<int, bool>* seen) {
  ...
  if (this->lines != nullptr) {  // List
    ...
    for (ListIter<SourceLine*> it(this->lines); !it.Done(); it.Next()) {
      SourceLine* v_ = it.Value();
      hnode_t* h = (v_ == nullptr) ? Alloc<hnode::Leaf>(StrFromC("_"),
                    color_e::OtherConst) : v_->PrettyTree(do_abbrev, seen);
      x4->children->append(h);
    }
    L->append(Alloc<Field>(StrFromC("lines"), x4));
  }

  if (this->right_tok) {  // Optional
    hnode_t* x5 = this->right_tok->PrettyTree(do_abbrev, seen);
    L->append(Alloc<Field>(StrFromC("right_tok"), x5));
  }
