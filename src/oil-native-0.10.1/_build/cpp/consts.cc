...
List<int>* OIL_BASIC = new List<int>({43, 41, 1, 42, 4, 2, 5, 35, 36, 37, 38, 39, 3, 44, 48, 47, 40, 6, 46, 45});
List<int>* OIL_ALL = new List<int>({43, 41, 1, 42, 4, 2, 5, 54, 35, 49, 50, 51, 36, 52, 53, 37, 38, 39, 3, 44, 48, 47, 55, 56, 57, 40, 26, 27, 28, 29, 30, 34, 31, 33, 32, 6, 46, 45});
List<int>* DEFAULT_TRUE = new List<int>({22, 41, 23, 42, 16, 50, 51, 52, 53, 48, 46});
List<int>* PARSE_OPTION_NUMS = new List<int>({42, 54, 35, 49, 50, 51, 36, 52, 20, 58, 53, 37, 38, 59, 39});
List<int>* SHOPT_OPTION_NUMS = new List<int>({25, 22, 65, 66, 67, 68, 69, 70, 71, 64, 43, 21, 72, 41, 73, 74, 75, 23, 19, 76, 42, 77, 18, 78, 17, 79, 80, 81, 82, 62, 83, 84, 63, 85, 4, 86, 60, 87, 88, 89, 90, 91, 92, 93, 94, 5, 54, 35, 49, 50, 51, 36, 52, 20, 58, 53, 37, 38, 59, 39, 44, 61, 95, 96, 24, 48, 97, 98, 47, 55, 56, 57, 40, 99, 26, 27, 28, 29, 30, 34, 31, 33, 32, 6, 100, 46, 45});
List<int>* SET_OPTION_NUMS = new List<int>({14, 1, 16, 11, 7, 10, 2, 3, 12, 9, 13, 8});
List<int>* VISIBLE_SHOPT_NUMS = new List<int>({25, 22, 43, 21, 41, 23, 19, 42, 18, 17, 4, 5, 54, 35, 49, 50, 51, 36, 52, 20, 58, 53, 37, 38, 59, 39, 44, 24, 48, 47, 55, 56, 57, 40, 26, 27, 28, 29, 30, 34, 31, 33, 32, 6, 46, 45});
...
builtin_t LookupNormalBuiltin(Str* s) {
  ...
  case '[':
    ...
    break;
  case '_':
    if (len == 5 && memcmp("__cat", data, 5) == 0) return 65;
    break;
  case 'a':
  ...
  case 'b':
    ...
    if (len == 10 && memcmp("boolstatus", data, 10) == 0) return 63;
    break;
  case 'c':
  ...
  case 'p':
    ...
    if (len == 14 && memcmp("push-registers", data, 14) == 0) return 64;
    break;
  case 'r':
