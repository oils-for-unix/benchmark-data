...
# Generated by build/native_graph.py
...
main() {
  ...
  link "$compiler" "$variant" "_bin/$compiler-$variant-sh/osh_eval" \
  ...
  if test "$variant" = opt; then
    strip -o "_bin/$compiler-$variant-sh/osh_eval.stripped" "_bin/$compiler-$variant-sh/osh_eval"
  fi
}
