...
class run {
 public:
  run(Dict<Str*, runtime_asdl::value_t*>* attrs) :
    allow_status_01(static_cast<value__Bool*>(attrs->index(new Str("allow_status_01")))->b),
    assign_status(attrs->index(new Str("assign_status"))->tag_() == value_e::Undef
      ? nullptr
      : static_cast<value__Str*>(attrs->index(new Str("assign_status")))->s),
    status_ok(attrs->index(new Str("status_ok"))->tag_() == value_e::Undef
      ? nullptr
      : static_cast<value__Str*>(attrs->index(new Str("status_ok")))->s) {
  }
