...
class hnode__ObjectCycle : public hnode_t {
 public:
  hnode__ObjectCycle(int heap_id)
      : heap_id(heap_id) {
  }

  static hnode__ObjectCycle* CreateNull(bool alloc_lists = false) { 
    return Alloc<hnode__ObjectCycle>(-1);
  }

  static constexpr ObjHeader obj_header() {
    return ObjHeader::AsdlClass(static_cast<uint16_t>(hnode_e::ObjectCycle), 0);
  }

  int heap_id;

  DISALLOW_COPY_AND_ASSIGN(hnode__ObjectCycle)
};
...
ASDL_NAMES hnode {
  typedef hnode__ObjectCycle ObjectCycle;
