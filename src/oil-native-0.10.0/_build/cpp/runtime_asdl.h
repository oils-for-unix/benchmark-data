...
class part_value__ExtGlob : public part_value_t {
 public:
  part_value__ExtGlob() :
                      ...
                      part_vals(new List<part_value_t*>()) {
  }
...
class assign_arg {
 public:
  assign_arg() : type_tag_(200), var_name(new Str("")), rval(nullptr),
...
class more_args {
 public:
  more_args() : type_tag_(201), typed_args(nullptr), block(nullptr) {
  }
  more_args(syntax_asdl::ArgList* typed_args, syntax_asdl::BraceGroup* block) :
            type_tag_(201), typed_args(typed_args), block(block) {
  }
  OBJ_HEADER();
  syntax_asdl::ArgList* typed_args;
  ...
  DISALLOW_COPY_AND_ASSIGN(more_args)
};

class VTestPlace {
 public:
  VTestPlace() : type_tag_(202), name(new Str("")), index(nullptr) {
  }
  VTestPlace(Str* name, a_index_t* index) : type_tag_(202), name(name),
             index(index) {
  }
  OBJ_HEADER();
  Str* name;
...
class VarSubState {
 public:
  VarSubState() : type_tag_(203), join_array(false), is_type_query(false) {
  }
  VarSubState(bool join_array, bool is_type_query) : type_tag_(203),
...
class cell {
 public:
  cell() : type_tag_(204), exported(false), readonly(false), nameref(false),
       val(nullptr) {
  }
  cell(bool exported, bool readonly, bool nameref, value_t* val) :
       type_tag_(204), exported(exported), readonly(readonly),
...
class redirect {
 public:
  redirect() : type_tag_(205), op_id(-1), op_spid(-1), loc(nullptr),
           arg(nullptr) {
  }
  redirect(Id_t op_id, int op_spid, syntax_asdl::redir_loc_t* loc,
           redirect_arg_t* arg) : type_tag_(205), op_id(op_id),
...
class Proc {
 public:
  Proc() : type_tag_(206), name(new Str("")), name_spid(-1), sig(nullptr),
  ...
  Proc(Str* name, int name_spid, syntax_asdl::proc_sig_t* sig,
       ...
       dynamic_scope) : type_tag_(206), name(name), name_spid(name_spid),
...
class StatusArray {
 public:
  StatusArray() : type_tag_(207), codes(new List<int>()), spids(new
              List<int>()) {
  }
  StatusArray(List<int>* codes, List<int>* spids) : type_tag_(207),
...
class CommandStatus {
 public:
  CommandStatus() : type_tag_(208), check_errexit(false), pipe_negated(false),
                pipe_status(new List<int>()), pipe_spids(new List<int>()),
                show_code(false) {
  }
  CommandStatus(bool check_errexit, bool pipe_negated, List<int>* pipe_status,
                List<int>* pipe_spids, bool show_code) : type_tag_(208),
