...
hnode_t* more_args::PrettyTree() {
  hnode__Record* out_node = runtime::NewRecord(new Str("more_args"));
  List<field*>* L = out_node->fields;
    if (this->typed_args) {  // MaybeType
      hnode_t* x0 = this->typed_args->PrettyTree();
      L->append(new field(new Str("typed_args"), x0));
    }

    if (this->block) {  // MaybeType
      hnode_t* x1 = this->block->PrettyTree();
      L->append(new field(new Str("block"), x1));
    }

  return out_node;
}

hnode_t* more_args::_AbbreviatedTree() {
  hnode__Record* out_node = runtime::NewRecord(new Str("more_args"));
  List<field*>* L = out_node->fields;
    if (this->typed_args) {  // MaybeType
      hnode_t* x0 = this->typed_args->AbbreviatedTree();
      L->append(new field(new Str("typed_args"), x0));
    }

    if (this->block) {  // MaybeType
      hnode_t* x1 = this->block->AbbreviatedTree();
      L->append(new field(new Str("block"), x1));
    }
...
hnode_t* more_args::AbbreviatedTree() {
  return _AbbreviatedTree();
}
