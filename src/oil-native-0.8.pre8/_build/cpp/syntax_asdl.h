...
namespace place_expr_e {
  const int Var = 1;
  const int Subscript = 1023;
  const int Attribute = 1024;
};
...
namespace class_literal_term_e {
  const int PosixClass = 1025;
  const int PerlClass = 1026;
  const int Range = 3;
...
class BraceGroup : public command_t {
 public:
  BraceGroup() : tag(1016), children(new List<command_t*>()), redirects(new
             List<redir*>()), spids(new List<int>()) {
  }
  BraceGroup(List<command_t*>* children, List<redir*>* redirects) : tag(1016),
...
class variant : public Obj {
 public:
  variant() : tag(1017), tag_name(nullptr), typ(nullptr) {
  }
  variant(Token* tag_name, variant_type_t* typ) : tag(1017),
...
class use_name : public Obj {
 public:
  use_name() : tag(1018), name(nullptr), alias(nullptr) {
  }
  use_name(Token* name, Token* alias) : tag(1018), name(name), alias(alias) {
  }
  uint16_t tag;
  Token* name;
  Token* alias;
...
class param : public Obj {
 public:
  param() : tag(1019), name(nullptr), type(nullptr), default_val(nullptr) {
  }
  param(Token* name, type_expr_t* type, expr_t* default_val) : tag(1019),
...
class name_type : public Obj {
 public:
  name_type() : tag(1020), name(nullptr), typ(nullptr) {
  }
  name_type(Token* name, type_expr_t* typ) : tag(1020), name(name), typ(typ) {
  }
  uint16_t tag;
  Token* name;
  type_expr_t* typ;
...
class comprehension : public Obj {
 public:
  comprehension() : tag(1021), lhs(new List<name_type*>()), iter(nullptr),
                cond(nullptr) {
  }
  comprehension(List<name_type*>* lhs, expr_t* iter, expr_t* cond) : tag(1021),
...
class named_arg : public Obj {
 public:
  named_arg() : tag(1022), name(nullptr), value(nullptr) {
  }
  named_arg(Token* name, expr_t* value) : tag(1022), name(name), value(value) {
  }
  uint16_t tag;
  Token* name;
  expr_t* value;
...
class subscript : public place_expr_t, public expr_t {
 public:
  subscript() : tag(1023), obj(nullptr), indices(new List<expr_t*>()) {
  }
  subscript(expr_t* obj, List<expr_t*>* indices) : tag(1023), obj(obj),
            indices(indices) {
  }
  uint16_t tag;
  expr_t* obj;
...
class attribute : public place_expr_t, public expr_t {
 public:
  attribute() : tag(1024), obj(nullptr), op(nullptr), attr(nullptr),
            ctx(expr_context_e::Load) {
  }
  attribute(expr_t* obj, Token* op, Token* attr, expr_context_t ctx) :
            tag(1024), obj(obj), op(op), attr(attr), ctx(ctx) {
  }
  uint16_t tag;
  expr_t* obj;
  Token* op;
...
class posix_class : public class_literal_term_t, public re_t {
 public:
  posix_class() : tag(1025), negated(nullptr), name(new Str("")) {
  }
  posix_class(speck* negated, Str* name) : tag(1025), negated(negated),
              name(name) {
  }
  uint16_t tag;
  speck* negated;
...
class perl_class : public class_literal_term_t, public re_t {
 public:
  perl_class() : tag(1026), negated(nullptr), name(new Str("")) {
  }
  perl_class(speck* negated, Str* name) : tag(1026), negated(negated),
             name(name) {
  }
  uint16_t tag;
  speck* negated;
