...
namespace option_i {
  ...
  const int more_errexit = 29;
  const int simple_test_builtin = 30;
  const int parse_at = 31;
  const int parse_brace = 32;
  const int parse_index_expr = 33;
  const int parse_paren = 34;
  const int parse_rawc = 35;
  const int parse_set = 36;
  const int parse_equals = 37;
  const int parse_do = 38;
