...
List<int>* STRICT_ALL = new List<int>({3, 22, 2, 21, 47, 4, 23, 24, 25, 34, 35, 26, 36, 27, 28, 29, 33, 30, 32, 31});
List<int>* OIL_BASIC = new List<int>({39, 38, 3, 22, 2, 21, 42, 43, 47, 44, 45, 46, 4, 40, 41, 37, 23, 24, 25, 26, 27, 28, 29, 33, 30, 32, 31});
List<int>* OIL_ALL = new List<int>({39, 38, 3, 22, 2, 21, 42, 50, 43, 49, 47, 44, 45, 46, 48, 4, 40, 41, 37, 23, 24, 25, 34, 35, 26, 36, 27, 28, 29, 33, 30, 32, 31});
List<int>* DEFAULT_TRUE = new List<int>({20, 38, 3, 47, 19});
List<int>* PARSE_OPTION_NUMS = new List<int>({42, 50, 43, 17, 49, 47, 44, 45, 46, 48, 51, 34, 35, 36});
List<int>* SHOPT_OPTION_NUMS = new List<int>({20, 58, 59, 60, 61, 62, 63, 64, 57, 39, 18, 65, 38, 66, 67, 68, 16, 69, 52, 70, 15, 71, 14, 72, 73, 74, 75, 55, 76, 77, 56, 78, 22, 79, 53, 80, 81, 82, 83, 84, 85, 86, 87, 21, 42, 50, 43, 17, 49, 47, 44, 45, 46, 48, 51, 40, 54, 88, 89, 90, 91, 41, 37, 92, 23, 24, 25, 34, 35, 26, 36, 27, 28, 29, 33, 30, 32, 31, 19, 93});
...
builtin_t LookupNormalBuiltin(Str* s) {
  ...
  case '[':
    ...
    break;
  case '_':
    if (len == 5 && memcmp("__cat", data, 5) == 0) return 59;
    break;
  case 'a':
  ...
  case 'f':
    ...
    if (len == 4 && memcmp("fork", data, 4) == 0) return 57;
    if (len == 8 && memcmp("forkwait", data, 8) == 0) return 58;
    break;
  case 'g':
  ...
  case 'r':
    ...
    if (len == 4 && memcmp("repr", data, 4) == 0) return 53;
    if (len == 3 && memcmp("run", data, 3) == 0) return 56;
    break;
  case 's':
