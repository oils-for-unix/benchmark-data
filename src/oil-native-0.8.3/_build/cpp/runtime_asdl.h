...
class job_status__Proc : public job_status_t {
 public:
  job_status__Proc() : type_tag_(static_cast<uint16_t>(job_status_e::Proc)),
                   code(-1) {
  }
  job_status__Proc(int code) :
                   type_tag_(static_cast<uint16_t>(job_status_e::Proc)),
                   code(code) {
  }
  OBJ_HEADER();
  int code;
...
class job_status__Pipeline : public job_status_t {
 public:
  job_status__Pipeline() :
                       type_tag_(static_cast<uint16_t>(job_status_e::Pipeline)), codes(new List<int>()) {
  }
  job_status__Pipeline(List<int>* codes) :
                       type_tag_(static_cast<uint16_t>(job_status_e::Pipeline)), codes(codes) {
  }
  OBJ_HEADER();
  List<int>* codes;
  hnode_t* PrettyTree();
...
class CompoundStatus {
 public:
  CompoundStatus() : type_tag_(203), negated(false), codes(new List<int>()),
                 spids(new List<int>()) {
  }
  CompoundStatus(bool negated, List<int>* codes, List<int>* spids) :
                 type_tag_(203), negated(negated), codes(codes), spids(spids) {
  }
  OBJ_HEADER();
  bool negated;
  List<int>* codes;
