...
class assign_arg : public Obj {
 public:
  assign_arg() : tag(200), var_name(new Str("")), rval(nullptr), spid(-1) {
  }
  assign_arg(Str* var_name, value_t* rval, int spid) : tag(200),
...
class cell : public Obj {
 public:
  cell() : tag(201), exported(false), readonly(false), nameref(false),
       val(nullptr) {
  }
  cell(bool exported, bool readonly, bool nameref, value_t* val) : tag(201),
...
class redirect : public Obj {
 public:
  redirect() : tag(202), op_id(-1), op_spid(-1), loc(nullptr), arg(nullptr) {
  }
  redirect(Id_t op_id, int op_spid, syntax_asdl::redir_loc_t* loc,
           redirect_arg_t* arg) : tag(202), op_id(op_id), op_spid(op_spid),
           loc(loc), arg(arg) {
  }
  uint16_t tag;
  uint16_t how_to_trace;
