...
GLOBAL_STR(gStr18, R"zZXx(Builtin Commands
...
GLOBAL_STR(gStr21, R"zZXx(Command Language
...
GLOBAL_STR(gStr22, R"zZXx(Expression Language and Assignments
...
GLOBAL_STR(gStr23, R"zZXx(Front End
...
GLOBAL_STR(gStr24, R"zZXx(Other Mini Languages

  [Patterns]      glob-pat      *.py
  [Other Sublang] braces        {alice,bob}@example.com
)zZXx");

GLOBAL_STR(gStr25, R"zZXx(Global Shell Options
...
GLOBAL_STR(gStr27, R"zZXx(Special Variables
...
GLOBAL_STR(gStr28, R"zZXx(Standard Library
...
GLOBAL_STR(gStr29, R"zZXx(Types and Methods
...
GLOBAL_STR(gStr31, R"zZXx(Word Language
...
GLOBAL_STR(gStr32, R"zZXx(YSH Command Language Keywords
...
GLOBAL_STR(gStr33, R"zZXx(# Can we define methods in pure YSH?
#
# (mylist->find(42) !== -1)
#
#   instead of 
...
GLOBAL_STR(gStr34, R"zZXx(# Bash strict mode, updated for 2024
...
GLOBAL_STR(gStr35, R"zZXx(# Library to turn a shell file into a "BYO test server"
#
# Usage:
#
...
GLOBAL_STR(gStr36, R"zZXx(#!/usr/bin/env bash
#
...
GLOBAL_STR(gStr38, R"zZXx(# Two functions I actually use, all the time.
#
...
GLOBAL_STR(gStr39, R"zZXx(# These were helpful while implementing args.ysh
...
GLOBAL_STR(gStr40, R"zZXx(# args.ysh
#
# Usage:
...
GLOBAL_STR(gStr41, R"zZXx(const __provide__ = :| Dict |
...
GLOBAL_STR(gStr42, R"zZXx(const __provide__ = :| any all sum repeat |
...
GLOBAL_STR(gStr43, R"zZXx(const __provide__ = :| identity max min abs |
...
GLOBAL_STR(gStr44, R"zZXx(# stream.ysh
#
# Usage:
...
GLOBAL_STR(gStr45, R"zZXx(# table.ysh - Library for tables.
#
# Usage:
...
GLOBAL_STR(gStr46, R"zZXx(#!/usr/bin/env bash
#
# Testing library for bash and OSH.
#
# Capture status/stdout/stderr, and nq-assert those values.
...
TextFile array[] = {
    ...
    {.rel_path = "stdlib/methods.ysh", .contents = gStr33},
    {.rel_path = "stdlib/osh/bash-strict.sh", .contents = gStr34},
    {.rel_path = "stdlib/osh/byo-server.sh", .contents = gStr35},
    {.rel_path = "stdlib/osh/no-quotes.sh", .contents = gStr36},
    {.rel_path = "stdlib/osh/task-five.sh", .contents = gStr37},
    {.rel_path = "stdlib/osh/two.sh", .contents = gStr38},
    {.rel_path = "stdlib/prelude.ysh", .contents = gStr39},
    {.rel_path = "stdlib/ysh/args.ysh", .contents = gStr40},
    {.rel_path = "stdlib/ysh/def.ysh", .contents = gStr41},
    {.rel_path = "stdlib/ysh/list.ysh", .contents = gStr42},
    {.rel_path = "stdlib/ysh/math.ysh", .contents = gStr43},
    {.rel_path = "stdlib/ysh/stream.ysh", .contents = gStr44},
    {.rel_path = "stdlib/ysh/table.ysh", .contents = gStr45},
    {.rel_path = "stdlib/ysh/yblocks.ysh", .contents = gStr46},
